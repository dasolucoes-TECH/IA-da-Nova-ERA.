...
import { Bot, Send, Sparkles, User, Plus, MessageSquare, Clock } from 'lucide-react'
import { Card, CardContent } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { sendAgentMessage, listAgentConversations, getAgentMessages } from '@/services/ai'
import { displayableMessages, type AgentMessage } from '@/lib/skipAi'
...
export default function IaPage() {
  ...
  const loadConversation = async (convId: string) => {
    try {
      const data = await getAgentMessages(convId)
      const display = displayableMessages((data as any).messages || [])
      setMessages(
        display.map((m) => ({ role: m.role as 'user' | 'assistant', content: m.content })),
      )
    ...
    <div className="h-[calc(100vh-8rem)] flex gap-4">
      <div className="hidden md:flex w-64 flex-col gap-2">
        <Button
          ...
          className="bg-[#FFC400] text-[#071B3B] font-bold hover:bg-amber-400 rounded-xl mb-2"
        >
        ...
        <div className="flex-1 overflow-y-auto space-y-1">
          ...
          {conversations.length === 0 ? (
            <p className="text-xs text-slate-400 px-2">Nenhuma conversa ainda.</p>
          ) : (
            conversations.map((c: any) => (
              <button
                key={c.id}
                onClick={() => loadConversation(c.id)}
                className={`w-full text-left p-2.5 rounded-xl text-xs transition-colors flex items-center gap-2 ${
                  conversationId === c.id
              ...
              >
                ...
                <span className="line-clamp-1">{c.title || 'Conversa sem título'}</span>
              </button>
            ))
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Agente conectado aos dados da loja. Alterações exigem aprovação.
          </p>
        </div>
            ...
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-6 max-w-lg mx-auto my-auto">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Selecione uma ação ou digite sua pergunta.
                  </p>
                </div>
              ...
              messages.map((m, idx) => (
                ...
                >
                  ...
                  <div
                    className={`p-3.5 rounded-2xl text-xs max-w-xl leading-relaxed whitespace-pre-wrap ${
                      m.role === 'user'
