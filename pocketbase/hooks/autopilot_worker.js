cronAdd('autopilot_worker', '* * * * *', () => {
  try {
    ...
    try {
      var staleJobs = $app.findRecordsByFilter(
        'automation_jobs',
        'status = {:st}',
        '-created',
        50,
          ...
          if (!product) {
            ...
            if (actionType === 'GENERATE_PRODUCT_SEO') {
              try {
                ...
                if (!d.seo_title || typeof d.seo_title !== 'string') throw new Error('Invalid SEO')
        ...
        if (autonomy === 'SUGGEST') {
          ...
          if (execResult.status === 'COMPLETED') {
            ...
            if (isRetryable && attempts < maxAttempts) {
              ...
              $app.save(job)
              try {
                var recentTerminal = $app.findRecordsByFilter(
                  'automation_jobs',
                  'rule = {:rid} && (status = {:s1} || status = {:s2} || status = {:s3})',
                  '-created',
                  5,
                  0,
                  { rid: rule.id, s1: 'COMPLETED', s2: 'FAILED', s3: 'CANCELLED' },
                )
                var allFailed = true
                for (var rj = 0; rj < recentTerminal.length; rj++) {
                  if (recentTerminal[rj].getString('status') !== 'FAILED') {
                    allFailed = false
                    break
                  }
                }
                if (allFailed && recentTerminal.length >= 5) {
                  ...
                  $app.save(rule)
                  try {
                    var nc = $app.findCollectionByNameOrId('automation_notifications')
                    var nr = new Record(nc)
                    nr.set('store', storeId)
                    nr.set('type', 'circuit_breaker')
