routerAdd('POST', '/backend/v1/autopilot/emit-event-core', (e) => {
  try {
    ...
    if (dedupKey) {
      try {
        var existing = $app.findFirstRecordByFilter('automation_events', 'deduplication_key = {:dk}', { dk: dedupKey })
        if (existing)
    ...
    function failEvent(rec, errMsg) {
      try {
        rec.set('status', 'FAILED')
        rec.set('processed_at', new Date().toISOString())
    ...
    try {
      var rules = []
      try {
        rules = $app.findRecordsByFilter('automation_rules', 'store = {:sid} && enabled = true && trigger_type = {:tt}', '-priority', 50, 0, { sid: storeId, tt: eventType })
      } catch (_) {}

      function getNestedValue(obj, path) {
        var parts = path.split('.'); var current = obj
      ...
      function evaluateCondition(cond, p) {
        ...
        switch (cond.operator) {
          ...
          case 'greater_or_equal': return Number(val) >= Number(cond.value)
          case 'less_than': return Number(val) < Number(cond.value)
          case 'less_or_equal': return Number(val) <= Number(cond.value)
          case 'contains': return String(val || '').indexOf(String(cond.value)) !== -1
          case 'not_contains': return String(val || '').indexOf(String(cond.value)) === -1
      ...
      function evaluateConditions(conditions, p) {
        ...
        if (conditions.all) {
          for (var i = 0; i < conditions.all.length; i++) { if (!evaluateCondition(conditions.all[i], p)) return false }
          return true
        }
        if (conditions.any) {
          for (var j = 0; j < conditions.any.length; j++) { if (evaluateCondition(conditions.any[j], p)) return true }
          return false
        }
      ...
      for (var r = 0; r < rules.length; r++) {
        ...
        var conditions = null
        try { conditions = JSON.parse(conditionsStr) } catch (_) {}
        ...
        if (cooldownMin > 0 && entityId) {
          ...
          try {
            var lastJobs = $app.findRecordsByFilter('automation_jobs', 'rule = {:rid} && (status = {:s1} || status = {:s2})', '-created', 50, 0, { rid: rule.id, s1: 'COMPLETED', s2: 'WAITING_APPROVAL' })
            for (var lj = 0; lj < lastJobs.length; lj++) {
              try {
                var ljEvent = $app.findRecordById('automation_events', lastJobs[lj].getString('event'))
                if (ljEvent.getString('entity_type') === entityType && ljEvent.getString('entity_id') === entityId) {
                  var lastDate = new Date(lastJobs[lj].getString('created'))
                  if (new Date(lastDate.getTime() + cooldownMin * 60000) > new Date(now)) { cooldownActive = true }
                  break
                }
              } catch (_) {}
            }
        ...
        if (maxPerDay > 0) {
          ...
          try {
            var todayJobs = $app.findRecordsByFilter('automation_jobs', 'rule = {:rid} && created >= {:ts} && (status = {:s1} || status = {:s2})', '-created', 200, 0, { rid: rule.id, ts: todayStart.toISOString(), s1: 'COMPLETED', s2: 'WAITING_APPROVAL' })
            if (todayJobs.length >= maxPerDay) {
              try {
                var nCol = $app.findCollectionByNameOrId('automation_notifications')
                var nRec = new Record(nCol)
                nRec.set('store', storeId); nRec.set('type', 'automation_limit')
                nRec.set('title', 'Automação pausada: limite diário atingido.')
                nRec.set('message', 'A automacao "' + rule.getString('name') + '" atingiu o limite de ' + maxPerDay + ' execucoes por dia.')
                nRec.set('severity', 'WARNING'); nRec.set('read', false); $app.save(nRec)
              } catch (_) {}
              continue
            }
          } catch (_) {}
        ...
        var idempotencyKey = storeId + ':' + rule.id + ':' + eventRec.id + ':' + (entityId || '') + ':' + rule.getString('action_type')
        try {
          var existingJob = $app.findFirstRecordByFilter('automation_jobs', 'idempotency_key = {:ik} && status = {:st}', { ik: idempotencyKey, st: 'COMPLETED' })
          if (existingJob) continue
        } catch (_) {}
        try {
          var dupJob = $app.findFirstRecordByFilter('automation_jobs', 'rule = {:rid} && event = {:eid}', { rid: rule.id, eid: eventRec.id })
          if (dupJob) continue
        } catch (_) {}

        var jobRec = new Record(jobsCol)
        jobRec.set('store', storeId); jobRec.set('rule', rule.id); jobRec.set('event', eventRec.id)
        jobRec.set('job_type', rule.getString('action_type')); jobRec.set('status', 'QUEUED')
        jobRec.set('priority', rule.getNumber('priority') || 5); jobRec.set('payload', JSON.stringify(payload))
        jobRec.set('attempts', 0); jobRec.set('max_attempts', 3); jobRec.set('scheduled_for', now)
      ...
      failEvent(eventRec, String(coreErr))
      $app.logger().error('emit_event_core_processing_error', 'error', String(coreErr), 'eventId', eventRec.id)
      return e.json(500, { error: 'Erro ao processar evento: ' + String(coreErr), eventId: eventRec.id })
    }
  } catch (err) {
